---
title: "Merging Suspensions with AFL Data"
output: pdf_document
---
...
for (i in 2:nrow(injuries)-1){
  if (injuries$Round[i] == injuries$Round[i+1] && injuries$Team[i] == injuries$Team[i+1] && injuries$Season[i] == injuries$Season[i+1] & injuries$Reason[i] == "Suspension"){
...
write.csv(injuries, "injuries_only_susp_grouped.csv")
```

```{r, eval=TRUE}
# Open Injury data
setwd("/Users/austinsechrest/Documents/GitHub/LISH-AFL-Project/Datasets")
injuries <- read.csv("injuries_only_susp_grouped.csv")
...
write.csv(afl, "afl_with_susp.csv")
```