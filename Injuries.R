...
injury_data <- data.frame(Season = character(), Date = character(), Round = numeric(), Team = character(), Opposition = character(), Out = character(), Reason = character())
...
for (h in 2:9){
	...
	for (i in 1:n){
	  ...
	  if (urls[i] == "https://www.afl.com.au/match-centre/2012/13/coll-v-wce#/summary"){
	    round <- url %>%
		...
		injuries <- url %>%
			...
			if (injuries1 != "NULL"){
				...
				for (j in 1:nrow(injuries1)){
					newrow <- data.frame(Season = season, Date = date, Round = round, Team = team1, Opposition = opposition1, Out = injuries1[j], Reason = reason1[j])
			...
			if (injuries2 != "NULL"){
				...
				for (j in 1:nrow(injuries2)){
					newrow <- data.frame(Season = season, Date = date, Round = round, Team = team2, Opposition = opposition2, Out = injuries2[j], Reason = reason2[j])
					injury_data <- rbind(injury_data, newrow)	
				}
			}
			counter = counter + 1
