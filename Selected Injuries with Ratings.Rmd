---
title: "Merging Non-Soreness Injuries"
output: pdf_document
---
...
for (i in 2:nrow(injuries)-1){
  if (injuries$Round[i] == injuries$Round[i+1] && injuries$Team[i] == injuries$Team[i+1] && injuries$Season[i] == injuries$Season[i+1] && injuries$Reason[i] != "Soreness" && injuries$Reason[i] != "Strain" && injuries$Reason[i] != "Rested" && injuries$Reason[i] != "Managed" && injuries$Reason[i] != "strain"){
...
write.csv(injuries, "selected_injuries_grouped.csv")
```

```{r, eval=TRUE}
# Open Injury data
setwd("/Users/austinsechrest/Documents/GitHub/LISH-AFL-Project/Datasets")
injuries <- read.csv("selected_injuries_grouped.csv")
...
write.csv(afl, "afl_with_selected_injuries.csv")
```